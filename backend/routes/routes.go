...
func AuthRoutes (db *gorm.DB, api fiber.Router) {
...
func PostRoutes(db *gorm.DB, api fiber.Router) {
...
func UploadRoutes(db *gorm.DB,  api fiber.Router) {
	api.Post("/upload", controllers.Upload)
}


func CommentsRoutes(db *gorm.DB, api fiber.Router) {
	api.Get("/comments",WithDB(controllers.GetComments,db))
	api.Post("/comments", WithDB(controllers.AddComment, db))
}

func LikesRoutes(db *gorm.DB, api fiber.Router) {
...
func FollowRequestRoutes(db *gorm.DB, api fiber.Router) {
